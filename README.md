...
|------|--------|------|:---:|:---:|:---:|:---:|
| 开发者 | dev_admin | 123456 | 全部 | ✅ | ✅ | ✅ 增删改+审批 |
| 分区管理者 | area_mgr | 123456 | 全部 | ✅ | ✅ | ✅ 增删改+审批 |
...
系统通过 `inspection_type` 字段实现三种角色数据分层：

| 角色 | 录入自动标记 | 可见范围 |
|------|------|------|
| 审核者 (INSPECTOR) | DAILY（日常检测） | 仅 DAILY |
| 管理者 (AREA_MANAGER) | SPOT_CHECK（抽检） | DAILY + SPOT_CHECK（开发者测试数据不可见） |
| 开发者 (DEVELOPER) | DEVELOPER（开发者测试） | 全部（DAILY + SPOT_CHECK + DEVELOPER） |

- 检测内容和指标模板三种角色完全一致
- 管理者抽检数据不计入日常检测统计，独立显示"本周抽检完成度"
- 开发者测试数据完全隔离：不污染仪表盘统计、不参与锁定逻辑、不在管理者/审核者视角出现
- 开发者以非测试身份录入（如替管理者抽检）时，需使用管理者账号
...
开发者/管理者操作：一键排班（全部产线排同一人）/ 逐线安排 / 取消
审核者操作：查看自己排班 / 申请变更（换人/取消）→ 管理者/开发者审批
审核者仅能在已排班的产线内进行手动录入
...
- 每次复测生成新 InspectionRecord（entryType=RETEST），自动继承原记录的 inspectionType
- 复测后正常/预警即锁定，不再允许对同一工位介质继续录入（开发者测试记录不参与锁定判断）
- 页面展示复测链 Timeline
- 后端 entry() 和 submitRetest() 均校验 latest.status≠3 时拒绝写入，同时排除 DEVELOPER 类型记录
...
按查看者角色分层展示：管理者看日常+抽检，审核者看日常，开发者看全部。异常条目标注发现方式（日常/抽检/开发者测试）。
...
| 角色数据隔离（DAILY/SPOT_CHECK/DEVELOPER 三层） | ✅ |
| 开发者权限（管理者权限超集 + 数据完全隔离） | ✅ |
| 知识库三级流程 | ✅ |
| 三级权限 + 路由守卫 | ✅ |
| 排班管理（含开发者排班） | ✅ |
| 系统管理 CRUD + 用户管理 | ✅ |
